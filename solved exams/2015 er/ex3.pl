...
seatdown(L,Seats1,Seats2) :-
        ...
        different_pairs(Seats1,Seats2,Seats2),
        %
...
different_pairs([],[], _).
different_pairs([_],[_], _).
different_pairs([X1,X2|Xs],[Y1,Y2|Ys], List2) :-
        (X1 #\= Y1 #/\ X1 #\= Y2) #\/ (X2 #\= Y1 #/\ X2 #\= Y2),
        checkdif([X1,X2], List2),
        different_pairs(Xs,Ys, List2).

checkdif([],[]).
checkdif([_,_], [_]).
checkdif([X1,X2], [Y1,Y2|Ys]):-
		 (X1 #\= Y1 #/\ X1 #\= Y2) #\/ (X2 #\= Y1 #/\ X2 #\= Y2),
		 checkdif([X1,X2], Ys).
